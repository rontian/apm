...
package com.apm.client.config
{
    ...
    public class RunConfig
    {
         ...
         */
        public function get appDirectory():String
        {
            return _appDirectory;
        }

        public function set appDirectory(value:String):void
        {
            _appDirectory = value;
        }
         ...
         */
        public function get airDirectory():String
        {
            return _airDirectory;
        }

        public function set airDirectory(value:String):void
        {
            _airDirectory = value;
        }
        ...
        public function loadEnvironment(callback:Function, checkNetwork:Boolean = false):void
        {
            ...
            if(checkNetwork)
            {
                _loadQueue.addProcess(new CheckNetworkProcess(this));
            }
        ...
        {
            var deployPackageCacheDir:File = DeployFileUtils.getDeployLocation(this, ProjectPackageCache.PACKAGE_CACHE_DIR);
        ...
        {
            var deployConfigDir:File = DeployFileUtils.getDeployLocation(this, "config");
        ...
        public function get airSdkCacheDirectory():File
        {
            ...
            if(!airSdkCacheDir.exists)
            {
                airSdkCacheDir.createDirectory();
            }
        ...
        public function getJava():File
        {
            ...
            throw new Error("Failed to find '" + javaBinPath + "' in JAVA_HOME=" + javaHome + ". Point JAVA_HOME to your java installation.");
        }

        public function getDefaultRemoteRepositoryEndpoint():String
        {
            var currentRepositeory:String = repository != null ? repository : APM.config.user.currentRepository;
            if(currentRepositeory != null && currentRepositeory.length > 0)
            {
                var repositories:Array = APM.config.user.repositoryList;
                for each(var repo:Object in repositories)
                {
                    if(repo.name == currentRepositeory)
                    {
                        Log.d(TAG, "Using config " + currentRepositeory + " repository: " + repo.url);
                        return repo.url;
                    }
                }
            }
